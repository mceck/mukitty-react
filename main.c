...
#define DEFAULT_WIDTH_CHARS 30  // Width in characters.
#define DEFAULT_HEIGHT_CHARS 10 // Height in characters.
#define DEFAULT_ZOOM 3          // Zoom level.
...
void kitty_update_display(long kitty_id, int frame_number, int width,
                          int height, uint8_t *fb) {
...
int process_input(mu_Context *ctx) {
  ...
  if (nread > 5 && strncmp(buf, "\033[<", 3) == 0) {
    ...
    if (sscanf(buf, "\033[<%d;%d;%d%c", &button, &x, &y, &event_type) == 4) {
      ...
      int pixel_x = (x - 1) * FONT_SIZE;
      int pixel_y = (y - 1) * FONT_SIZE;
...
void test_window(mu_Context *ctx) {
  if (mu_begin_window(ctx, "Log Window", mu_rect(10, 10, 380, 280))) {
    /* output text panel */
